...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if "Qwen2-VL" in model_id or "qwen2-vl" in model_id:
        ...
        # 异步写入器
        # 初始化异步日志记录器
        if self.is_world_process_zero() and os.getenv("DEBUG_MODE") == "true":
            log_path = os.getenv("LOG_PATH")
            self.async_logger = AsyncLogger(log_path)
        else:
            self.async_logger = None
    
    def cleanup(self):
        # 在训练结束时关闭异步日志记录器
        if hasattr(self, 'async_logger') and self.async_logger is not None:
            self.async_logger.close()
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        for x in inputs:
            image = []
            image.append(Image.open(x["images"][0]))
            images.append(image)
        ...
        if self.is_world_process_zero() and os.getenv("DEBUG_MODE") == "true":
            ...
            for idx, completion in enumerate(completions):
                iid = idx % len(inputs)
                gt = inputs[iid].get("solution")
                acc, fmt = rewards_cpu[idx, :2].tolist()
                
                log_entry = (
                    f"--- {current_time} --- Step {self.state.global_step} --- Sample {idx} --- "
                    f"Accuracy: {acc:.2f} --- Format: {fmt:.2f} ---\n"
                    f"Output:\n{completion[0]['content']}\n"
                    f"Ground truth: {gt}\n"
                )
                log_data.append(log_entry)

            self.async_logger.log("".join(log_data))
